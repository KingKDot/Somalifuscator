...
namespace SomalifuscatorV3.Methods.WalkThroughObfuscation
{
    public struct CodeBlock
    {
        ...
        public CodeBlock(string label, IEnumerable<string> commands, BlockType type = BlockType.Standard)
        {
            Label = label ?? throw new ArgumentNullException(nameof(label));
            Commands = commands?.ToList().AsReadOnly() ?? throw new ArgumentNullException(nameof(commands));
            Type = type;
        }

        public CodeBlock(string label, BlockType type = BlockType.Standard) : this(label, new List<string>(), type) { }

        public CodeBlock AddCommand(string command)
        {
            var newCommands = Commands.ToList();
            newCommands.Add(command);
            return new CodeBlock(Label, newCommands, Type);
        }

        public CodeBlock AddCommands(IEnumerable<string> commands)
        {
            var newCommands = Commands.ToList();
    ...
    public struct ObfuscationVariable
    {
        public string Name { get; }
        public string Value { get; }

        public ObfuscationVariable(string name, string value)
        {
    ...
    {
        public bool UseBlockObfuscation { get; }
        public int InitialVariableCount { get; }
        public int VariableGenerationBlocks { get; }
        public double VariableInsertionProbability { get; }

        public ObfuscationOptions(bool useBlockObfuscation = true, int initialVariableCount = 100,
                                int variableGenerationBlocks = 100, double variableInsertionProbability = 0.5)
        {
            UseBlockObfuscation = useBlockObfuscation;
            InitialVariableCount = initialVariableCount;
            VariableGenerationBlocks = variableGenerationBlocks;
            VariableInsertionProbability = Math.Clamp(variableInsertionProbability, 0.0, 1.0);
        }
    }
    ...
    namespace SomalifuscatorV3.Methods.WalkThroughObfuscation
    {
        public class Walk
        {
            ...
            private string WalkThroughLinear(List<string> lines)
            {
                ...
                if (Settings.UseVariableGeneration)
                {
                    int varCount = _random.Next(10, 30);
                    for (int i = 0; i < varCount; i++)
                    {
                        AddGeneratedVariable(blockGroup);
                    }
                }
                ...
                foreach (string line in lines)
                {
                    ...
                    if (Settings.UseVariableGeneration && _random.Next(0, 4) == 0)
                    {
                        AddGeneratedVariable(blockGroup);
                    }
                }
            ...
            private string WalkThroughWithBlockObfuscation(List<string> lines)
            {
                ...
                int variableBlockCount = Settings.UseVariableGeneration ? 100 : 0;

                for (int i = 0; i < variableBlockCount; i++)
                {
                    var blockGroup = new BlockGroup();
                ...
                for (int i = 0; i < lines.Count; i++)
                {
                    ...
                    blockGroup.AddBlock($":{pointers[i + variableBlockCount]}");

                    if (Settings.UseVariableGeneration && _random.Next(0, 2) == 0)
                    {
                        AddGeneratedVariable(blockGroup);
                    }
                    ...
                    int nextPointer = (i == lines.Count - 1) ? pointers[pointers.Count - 1] : pointers[i + variableBlockCount + 1];
                    string obfuscatedNumber = ObfuscateNumbers.ObfuscateNumber(nextPointer.ToString());
                    blockGroup.AddBlock(ObfuscateLineWithStart($"set /a ans={obfuscatedNumber}"));
                    blockGroup.AddBlock(ObfuscateLineWithStart("goto") + " %ans%");
            ...
            private void AddGeneratedVariable(BlockGroup blockGroup)
            {
                var (outVariable, variableName, variableValue) = VariableGenerator.GenerateVariable();
                blockGroup.AddBlock(ObfuscateLineWithStart(outVariable.TrimEnd()));
            ...
            {
                if (!Settings.UseLinePrefixObfuscation)
                {
                    return string.Empty;
                }
            ...
            {
                if (!Settings.UseBlockObfuscation || !Settings.UseBlockShuffling)
                    return;
